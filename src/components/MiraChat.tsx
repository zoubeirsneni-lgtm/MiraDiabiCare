...
export default function MiraChat({ profile, logs }: MiraChatProps) {
  ...
  return (
    <div className="max-w-4xl mx-auto h-[calc(100vh-16rem)] flex flex-col bg-white rounded-[40px] shadow-sm border border-gray-50 overflow-hidden">
      {/* Header */}
      <div className="p-6 border-b border-gray-100 bg-gradient-to-r from-medical-blue/5 to-transparent flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h3 className="text-xl font-serif font-bold text-gray-900">Mira AI</h3>
        ...
        <div className="hidden md:flex gap-2">
          {suggestions.slice(0, 2).map((s, i) => (
            <button 
              key={i}
              onClick={() => setInput(s)}
              className="px-4 py-2 bg-white rounded-xl border border-gray-100 text-xs font-medium text-gray-500 hover:border-medical-blue hover:text-medical-blue transition-all"
            >
              {s}
            </button>
          ))}
